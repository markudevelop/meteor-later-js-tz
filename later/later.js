  ...
  later.date.timezone = function(timezone) {
    ...
    if (timezone && timezone !== 'local') {
      if (Meteor.isServer) {
        ...
        console.log('timezone support is server-side only!');
      }
    } else {
      // old logic
      later.date.build = timezone ? function(Y, M, D, h, m, s) {
        return new Date(Y, M, D, h, m, s);
      } : function(Y, M, D, h, m, s) {
        return new Date(Date.UTC(Y, M, D, h, m, s));
      };
      var get = timezone ? "get" : "getUTC", d = Date.prototype;
